...
local theme_file = (os.getenv("XDG_CONFIG_HOME") or os.getenv("HOME") ..
    "/.config") .. "/somewm/theme"
...
do
    ...
    }) do beautiful[key] = gears.color.recolor_image(layouts_dir .. file, fg) end
end
...
local mymainmenu = awful.menu({
    items = {
        { "somewm",        menu_items, beautiful.awesome_icon },
...
tag.connect_signal("request::default_layouts", function()
    awful.layout.append_default_layouts({
        awful.layout.suit.tile, awful.layout.suit.fair, awful.layout.suit.max,
...
local mytextclock = wibox.widget {
    {
        { format = "%a %b %d  %H:%M", widget = wibox.widget.textclock },
        left = dpi(8),
        right = dpi(8),
...
local cal_popup
do
    local ok, calendar = pcall(require, "widgets.calendar")
    if ok then
        cal_popup = calendar.popup
    else
        naughty.notification {
            urgency = "critical",
            title   = "Calendar widget error",
...
cal_popup:attach(mytextclock, "tc", { on_hover = false })
...
screen.connect_signal("request::desktop_decoration", function(s)
    ...
    if restore then
        ...
        awful.tag({ "home", "web", "fun", "emacs" }, s, awful.layout.layouts[1])
    end
    ...
    s.mywibox = awful.wibar {
        ...
        widget = {
            ...
            { mytextclock, halign = "center", widget = wibox.container.place }
        }
    }
end)
...
ruled.client.connect_signal("request::rules", function()
    ...
    ruled.client.append_rule {
        ...
        callback = function(c)
            c.hidden = true
            local function resolve(client)
                local name = client.name or ""
                if name == "" or name == "Mozilla Firefox" then
                    return
                end
                client:disconnect_signal("property::name", resolve)
                if name:match("Bitwarden") then
            ...
            c:connect_signal("property::name", resolve)
            gears.timer.start_new(0.3, function()
                if c.valid and c.hidden then
                    c:disconnect_signal("property::name", resolve)
                    c.hidden = false
                end
                return false
            end)
